...
sub trimDeletedFiles{
	...
	my $allFilesQuery = qq/SELECT $params::photoFileColumn, $params::rootDirNumColumn, $params::photoKeyColumn FROM $params::photoTableName/;
	...
	while($query->fetch){
		...
		if (! -e $file){
			...
			my $unlinkQuery = qq/DELETE FROM $params::linkerTableName WHERE $params::linkerPhotoColumn = $photoKey/;
			...
			my $deletePhotoQuery = qq/DELETE FROM $params::photoTableName WHERE $params::photoFileColumn = "$fileName" AND $params::rootDirNumColumn = $rootDirNum/;
